...
import carmove.testmove.TestMoveControll3D;
...
//		pnlScreen = new JPanel();
		...
		rbnGroup.add( rbnControll3D = new JRadioButton("3DëÄçÏ") );
		tm_select = 0;
		...
		pnlOption.add(rbnControll3D, BorderLayout.CENTER);
...
//				g.clearRect(0,0,2000,2000);
		...
		rbnControll3D.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				rbnGroup.selectButton(6);
			}
		});
...
//		g.fillPolygon(sx,sy,3);
	...
	public void move3D(int b,int a,double h, SceneryMap map){
		...
		System.out.println("ç¿ïW(X,Y)=("+(int)car.getX()+","+(int)car.getY()+") å¸Ç´="+car.getDirection());
		
		double x = car.getX();
		double y = car.getY();
//		double d = (double)car.getDirection() * Math.PI / 180.0;
		double d = car.getDirection();
//		map.drawScenery(x, y, d, g);
		map.drawScenery(x, y, d, offg);
		pnlScreen.repaint();
		if(sw_sleep){
			try{
				Thread.sleep(10);
	...
	public void drawCar(){
		switch(tm_select){
			...
			case 5:
				...
				break;
			case 6:
				tm = new TestMoveControll3D(this, XMAX);
				break;
			default:
