public class Kadai2030{
    public static void main(String[] args){
        
        ObjectListG<MemberEx> members = new ObjectListG<MemberEx>();
        
        String prompt = "[e:終了, n:新規, a:一覧, r:貸出, b:返却]";
        char cmd = 0;
        while((cmd = Keyboard.readChar(prompt)) != 'e'){
            switch(cmd){
            case 'n':
                System.out.println("新会員登録");
                int kind = 0;
                do{
                    kind = Keyboard.readInt("種別[0:通常, 1:特別]");
                }while(kind < 0 || kind > 1);
                switch(kind){
                case 0:
                    members.add(members.size(), MemberEx.newMember());
                    break;
                case 1:
                    members.add(members.size(), SPMember.newMember());
                    break;
                }
                break;
            case 'a':
                Object[] list = members.toArray();
                for(int i = 0; i < list.length; i++){
                    System.out.println(list[i]);
                }
                break;
            case 'r':
                MemberEx m = search(members);
                if(m != null){
                    m.rent();
                }
                break;
            case 'b':
                m = search(members);
                if(m != null){
                    m.back();
                }
                break;
            default:
            }
            System.out.println();
        }
    }
    
    /**
     * 入力した会員番号のインスタンスを探索
     */
    private static MemberEx search(ObjectListG<MemberEx> members){
        int no = Keyboard.readInt("会員番号");
        Object[] list = members.toArray();
        int i = 0;
        while(i < list.length && ((MemberEx)list[i]).getNo() != no){
            i++;
        }
        if(i >= list.length){
            System.out.println("その番号の会員は存在しません。");
            return null;
        }
        return (MemberEx)list[i];
    }
}
