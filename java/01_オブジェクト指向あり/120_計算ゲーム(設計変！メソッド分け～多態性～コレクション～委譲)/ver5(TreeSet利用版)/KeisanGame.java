...
public class KeisanGame{
    ...
    private static final String  FILEPATH = "score.dat";                                  //スコアファイル名
    
    //-----コンストラクタ：各メンバの初期化とゲーム進行管理メソッドの呼び出し-------------
    public KeisanGame(){
        games = ArrayListFiling.read(FILEPATH);                                           //ファイルからスコア情報を読み込み
        if(games == null){
            games = new ArrayList<GameType>();                                            //読み込み不可ならスコアを新規作成
            games.add(new AddGame());
    ...
    public void mainGame(){
        try{
            while(true){
                ...
                ArrayListFiling.write(games, FILEPATH);                                   //スコアをファイルに保存
            }
