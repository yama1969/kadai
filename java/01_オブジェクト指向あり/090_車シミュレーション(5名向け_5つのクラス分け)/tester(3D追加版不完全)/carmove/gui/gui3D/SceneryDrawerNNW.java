...
public class SceneryDrawerNNW implements SceneryDrawer{
	public void drawScenery(double x, double y, double direction, SceneryMap map, Graphics g){
		...
		//北西～北を向いている
		//  Y座標の大から小(添字の小から大)へ、X座標の小から大へ描く
		...
		for(int map_y = 0; map_y < map_y_max + 1; map_y++){
			...
			double space_x_max = ((double)space_y - y) * r_slope + x;
			double space_x_min = ((double)space_y - y) * l_slope + x;
			...
			//X座標の小から大へ、オブジェクトを順番に取り出す
			for(int map_x = map_x_min; map_x < map_x_max + 1; map_x++){
				...
				if(objects[map_y][map_x] != null){
					...
					if(faces[SceneryObject.EAST] != null){
						for(int i = 0; i < faces[SceneryObject.EAST].length; i++){
							SceneryTrans.draw(x, y, map.getViewZ(), direction, width, height, depth, space_x, space_y, faces[SceneryObject.EAST][i], g);
						}
					}
