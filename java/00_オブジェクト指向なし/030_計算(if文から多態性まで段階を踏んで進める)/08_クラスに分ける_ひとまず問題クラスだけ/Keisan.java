...
public class Keisan{
    ...
    private int[][] rank = new int[KIND_NUM][RANK_NUM];               //ランキング用配列(得点)
    private String[][] name = new String[KIND_NUM][RANK_NUM];         //ランキング用配列(氏名)
    ...
    public void execKeisan(){
        ...
        Question q = new Question();                             //計算問題オブジェクト
        do{
            ...
            if(kind != END){
                ...
                for(int i = 0; i < QUES_NUM; i++){
                    q.makeQuestion(kind);                        //問題を作る
                    int ans = 0;
                    do{
                        q.showQuestion(i, kind);                 //問題を表示する
                        ans = inputAns();                        //回答を入力する
                    }while(malinput);
                    correct += q.judge(ans, kind);               //正誤を判定する
                }
     ...
     * その他：ランキング配列を更新する
     **************************************************************************/
    public void ranking(int score, int kind){
        System.out.println();
     ...
     * その他：なし
     **************************************************************************/
    public void showRanking(int kind){
